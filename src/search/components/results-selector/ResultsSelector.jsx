...
const ResultsSelector = ({
  ...
  return kind === "select" ? (
    <div className={`results-selector ${className}`}>
      <label className="results-selector-label" htmlFor={id}>
        {labelText}
      </label>
      <select
        className="results-selector-input"
        id={id}
        value={value}
    ...
    <ul className={`results-selector-list ${className}`}>
      ...
      {options.map(({ itemValue, itemText }) => (
        <li
          ...
          className={`results-selector-list-item ${
            itemValue === value ? "selected" : ""
          }`}
          onClick={() => {
            handleListItemClick(itemValue);
          }}
          onKeyDown={(event) => handleListItemKeyDown(event, itemValue)}
        >
          {itemText}
          {itemValue === value && (
            <button onClick={handleRemoveLanguageButtonClick}>x</button>
          )}
        </li>
      ))}
    </ul>
