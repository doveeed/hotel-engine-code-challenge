...
const ResultsItem = ({item}) => {
    ...
    const handleNameClick = () => {
        history.push(`/details/${encodeURIComponent(full_name)}`, { repository: item, callbackPath: `${pathname}${search}`})
    }
    
    return (<div className="results-item">
        <hr></hr>
        <div className="results-item-name" tabIndex="0" onClick={handleNameClick}>{full_name}</div>
